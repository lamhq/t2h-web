...
const StyledSpinner = styled.svg<SpinnerProps>`
  ...
  & .path {
    stroke: ${({ theme }) => theme.colors.loadingSpinner};
...
type SpinnerProps = { size?: number; strokeWidth?: number; isAutoCentering?: boolean };
...
const Spinner: React.FC<SpinnerProps> = (props: SpinnerProps) => {
  ...
  return (
    <StyledSpinner size={size} isAutoCentering={isAutoCentering} viewBox={`0 0 ${size} ${size}`}>
      <circle className="path" cx={size / 2} cy={size / 2} r={size / 2 - strokeWidth / 2} fill="none" strokeWidth={strokeWidth} />
    </StyledSpinner>
  );
};
